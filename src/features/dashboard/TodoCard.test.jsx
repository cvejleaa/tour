...
describe('TodoCard', () => {
  ...
  it('viser "alt besvaret" når total er 0', () => {
    mockTasks.mockReturnValue({ stageCount: 0, bonusCount: 0, leagueBonus: { total: 0, byLeague: [] }, total: 0 });
    renderCard();
  ...
  it('skjuler kategorier uden mangler', () => {
    mockTasks.mockReturnValue({ stageCount: 0, bonusCount: 2, leagueBonus: { total: 0, byLeague: [] }, total: 2 });
    renderCard();
    expect(screen.queryByText(/mangler tip/)).not.toBeInTheDocument();
    expect(screen.getByText(/bonusspørgsmål åbne/)).toBeInTheDocument();
  });
