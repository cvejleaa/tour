// functions-platform/startGate.js — HVOR ET SPIL BEGYNDER.
//
// SPEJL af src/lib/startGate.js. De to filer skal følges ad (CLAUDE.md), og
// pariteten er testdækket i startGate.test.js.
//
// Serveren er eneste autoritet: den afgør, hvilke kampe der giver point.
// Klienten bruger samme modul, så den viser præcis de kampe, der tælles.
//
 ...
 */
function foerStart(match, startRunde) {
 ...
 * @returns {Set<string>}
 */
function gatedeKampe(matches, startRunde) {
  const ud = new Set();
...
function fraStartRunde(matches, startRunde) {
 ...
 */
function rundeForTidspunkt(matches, startMs) {
 ...
 */
function startRundeFor(game, matches) {
